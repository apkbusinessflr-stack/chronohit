...
export default async function handler(req) {
  try{
    ...
    // fallback for other games you may add later (tr/ss)
...
// TAP (avg lower is better => store positive avg)
...
// SB (higher score is better => store NEGATIVE score to keep zrange ascending)
async function pushLeaderboardSB(day, item){
  ...
  const member = JSON.stringify({
    score:item.score, correct:item.correct, wrong:item.wrong,
    streakMax:item.streakMax, accuracy:item.accuracy, mode:item.mode
  });
  const score = -item.score; // store negative
  const url = `${UPSTASH_REDIS_REST_URL}/zadd/${encodeURIComponent(zkey)}/${score}/${encodeURIComponent(member)}`;
  await fetch(url, { headers: { Authorization: `Bearer ${UPSTASH_REDIS_REST_TOKEN}` } });
  const ttl = `${UPSTASH_REDIS_REST_URL}/expire/${encodeURIComponent(zkey)}/604800`;
  await fetch(ttl, { headers: { Authorization: `Bearer ${UPSTASH_REDIS_REST_TOKEN}` } });
}
