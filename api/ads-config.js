...
export default async function handler(req, res) {
  try{
    ...
    const tr   = process.env.ADSENSE_SLOT_TARGETRUSH || "";
    const ss   = process.env.ADSENSE_SLOT_SEQUENCESPRINT || "";
    const sb   = process.env.ADSENSE_SLOT_STROOP || "";
    if (!pub) return res.status(200).json({ enabled:false, reason:"Missing ADSENSE_PUBLISHER_ID" });
    return res.status(200).json({
      enabled:true,
      publisher: pub,
      slots: { home, tapreflex: tap, targetrush: tr, sequencesprint: ss, stroop: sb }
    });
  }catch(e){
